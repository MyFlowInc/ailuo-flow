...
export const selectCurContractForm = (state: RootState) =>
	state.global.curContractForm;
...
export const selectIsPurchase = (state: RootState) => {
	const { user } = state.global;
	const { roles } = user;
	let res = false;
	if (!roles) {
